...
def record_optical_flow():
    ...
    if args.csv is not None:
        if args.csv == "auto":
            ...
            csv_path = os.path.join("recordings", f"optical_flow_session_{time.strftime('%Y%m%d_%H%M%S')}.csv")
        else:
            csv_path = args.csv
    elif args.record:
        os.makedirs("recordings", exist_ok=True)
    ...
    if csv_path:
        try:
            ...
            csv_file.flush()
            print(f"Logging session sensor readings and trajectory to CSV: {csv_path}")
        except Exception as e:
    ...
    try:
        while True:
            ...
            # Write current frame sensor reading and trajectory sample to CSV file
            if csv_writer is not None:
                try:
                    csv_writer.writerow([
                        ...
                        f"{vx_raw_mps:.4f}",
                        f"{vy_raw_mps:.4f}",
                        f"{velocity_state.get('speed_mps', 0.0):.4f}",
                        f"{current_alt:.4f}",
                        f"{roll_deg:.4f}",
                        f"{pitch_deg:.4f}",
                        f"{yaw_deg:.4f}",
        ...
        if csv_file is not None:
            try:
                csv_file.close()
                print(f"\nCSV session log saved successfully: {csv_path}")
            except Exception as e:
