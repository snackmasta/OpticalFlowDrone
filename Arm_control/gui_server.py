...
<body>
    <div class="container">
        ...
        <div class="slider-card" style="border: 1px solid #89b4fa; background-color: #2a2b3d;">
            <div class="slider-header">
                <span style="color: #89b4fa; font-weight: bold;">↔️ Lateral Reach (Opposite J2 & J3)</span>
                <span class="angle-val" id="valLat" style="color: #89b4fa;">0</span>
            </div>
            <input type="range" id="sLat" min="-45" max="45" value="0" oninput="onLateralChange()">
        </div>

        <div class="slider-card" style="border: 1px solid #a6e3a1; background-color: #2a2b3d;">
            <div class="slider-header">
                <span style="color: #a6e3a1; font-weight: bold;">🔄 Synchronized Pitch (Same Dir J2 & J3)</span>
                <span class="angle-val" id="valSync" style="color: #a6e3a1;">0</span>
            </div>
            <input type="range" id="sSync" min="-45" max="45" value="0" oninput="onSyncChange()">
        </div>
    ...
    <script>
        ...
        function setPresets(a1, a2, a3, a4) {
            document.getElementById('sLat').value = 0;
            document.getElementById('valLat').innerText = 0;
            document.getElementById('sSync').value = 0;
            document.getElementById('valSync').innerText = 0;
        ...
        function onLateralChange() {
            const offset = parseInt(document.getElementById('sLat').value, 10);
            document.getElementById('valLat').innerText = offset > 0 ? `+${offset}` : offset;
            
            // Opposite coupling: Shoulder pitch and Elbow pitch move in opposite directions
            let newShoulder = Math.max(0, Math.min(180, baseShoulder + offset));
            let newElbow = Math.max(0, Math.min(180, baseElbow - offset));

            document.getElementById('s2').value = newShoulder; // Shoulder J2
            document.getElementById('s1').value = newElbow;    // Elbow J3
            sendAngles();
        }

        function onSyncChange() {
            const offset = parseInt(document.getElementById('sSync').value, 10);
            document.getElementById('valSync').innerText = offset > 0 ? `+${offset}` : offset;
            
            // Synchronized coupling: Shoulder pitch and Elbow pitch move in the SAME direction
            let newShoulder = Math.max(0, Math.min(180, baseShoulder + offset));
            let newElbow = Math.max(0, Math.min(180, baseElbow + offset));
