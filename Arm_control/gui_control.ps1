...
$form.Size = New-Object System.Drawing.Size(530, 750)
...
$groupLat = Create-SliderGroup -Name "Lateral Reach" -PinLabel "Opposite J2 and J3" -YPos 95
$groupLat.Track.Minimum = -45
$groupLat.Track.Maximum = 45
$groupLat.Track.Value = 0

$groupSync = Create-SliderGroup -Name "Synchronized Pitch" -PinLabel "Same Dir J2 and J3" -YPos 170
$groupSync.Track.Minimum = -45
$groupSync.Track.Maximum = 45
$groupSync.Track.Value = 0

$group1 = Create-SliderGroup -Name "Servo 1" -PinLabel "GPIO 1 / Elbow Pitch (J3)" -YPos 245
$group2 = Create-SliderGroup -Name "Servo 2" -PinLabel "GPIO 3 / Shoulder Pitch (J2)" -YPos 320
$group3 = Create-SliderGroup -Name "Servo 3" -PinLabel "GPIO 5 / Base Yaw (J1)" -YPos 395
$group4 = Create-SliderGroup -Name "Servo 4" -PinLabel "GPIO 4 / Wrist Pitch (J4)" -YPos 470

$script:baseShoulder = 90
$script:baseElbow = 90

# Event handler for direct slider changes
$updateAngles = {
...
# Event handler for lateral reach coupled control (Opposite directions)
$updateLateral = {
    $offset = $groupLat.Track.Value
    $groupLat.Label.Text = "Lateral Reach (Opposite J2 and J3): " + ($(if ($offset -gt 0) { "+$offset" } else { "$offset" }))

    $newShoulder = [Math]::Max(0, [Math]::Min(180, $script:baseShoulder + $offset))
    $newElbow = [Math]::Max(0, [Math]::Min(180, $script:baseElbow - $offset))

    $group2.Track.Value = $newShoulder
    $group1.Track.Value = $newElbow

    & $updateAngles
}

# Event handler for synchronized pitch coupled control (Same direction)
$updateSync = {
    $offset = $groupSync.Track.Value
    $groupSync.Label.Text = "Synchronized Pitch (Same Dir J2 and J3): " + ($(if ($offset -gt 0) { "+$offset" } else { "$offset" }))
...
$groupLat.Track.Add_Scroll($updateLateral)
$groupSync.Track.Add_Scroll($updateSync)
...
$panelPresets.Location = New-Object System.Drawing.Point(20, 555)
...
Create-Button -Text "0${deg} (Min)" -X 0 -Width 108 -Action {
    $groupLat.Track.Value = 0; $groupSync.Track.Value = 0; $group1.Track.Value = 0; $group2.Track.Value = 0; $group3.Track.Value = 0; $group4.Track.Value = 0
    & $updateAngles
}

Create-Button -Text "90${deg} (Neutral)" -X 118 -Width 118 -Action {
    $groupLat.Track.Value = 0; $groupSync.Track.Value = 0; $group1.Track.Value = 90; $group2.Track.Value = 90; $group3.Track.Value = 90; $group4.Track.Value = 90
    & $updateAngles
}

Create-Button -Text "180${deg} (Max)" -X 246 -Width 108 -Action {
    $groupLat.Track.Value = 0; $groupSync.Track.Value = 0; $group1.Track.Value = 180; $group2.Track.Value = 180; $group3.Track.Value = 180; $group4.Track.Value = 180
    & $updateAngles
}
