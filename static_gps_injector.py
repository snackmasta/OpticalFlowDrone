#!/usr/bin/env python3
"""
Inject static fake GPS telemetry (GPS_RAW_INT) into ArduPilot for proof-of-concept testing.
Matches ArduPilot's GPS_RAW_INT MAVLink structure for maximum compatibility.
...
def main():
    parser = argparse.ArgumentParser(description="Inject static fake GPS_RAW_INT telemetry into ArduPilot.")
    parser.add_argument(
        "--connection",
        type=str,
    ...
    print(f"Starting GPS_RAW_INT injection loop at {args.rate}Hz...")
    ...
    try:
        while True:
            ...
            speed_mps = math.hypot(args.vn, args.ve)
            vel_cm_s = int(speed_mps * 100)
            
            if speed_mps > 0.05:
                cog_deg = math.degrees(math.atan2(args.ve, args.vn))
            ...
            try:
                master.mav.gps_raw_int_send(
                    ...
                    int(args.alt * 1000),       # Altitude in mm (AMSL)
                    100,                        # HDOP * 100 (e.g. 100 = 1.0)
                    100,                        # VDOP * 100 (e.g. 100 = 1.0)
                    vel_cm_s,                   # GPS ground speed in cm/s
                    cog_cd,                     # Course over ground in degrees * 100
                    args.satellites,            # Satellites visible
                    int(args.alt * 1000),       # Altitude (ellipsoid) in mm
            ...
            if now - last_print_time >= 2.0:
                print(f"Status: Sent {packets_sent} GPS_RAW_INT packets. Fix={args.fix_type}, Sats={args.satellites}.")
                last_print_time = now
